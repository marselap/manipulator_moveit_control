...
#include "ros/ros.h"
#include <string>
#include <manipulator_moveit_control/PID.h>
...
#include <manipulator_moveit_control/JointCtlParamsConfig.h>

class JointControl
{
	private:
		void configCallback(manipulator_moveit_control::JointCtlParamsConfig &config, uint32_t level);
		float ref_;
		bool config_start_;
		PID joint_pid_;

		dynamic_reconfigure::Server<manipulator_moveit_control::JointCtlParamsConfig> *dr_srv;
		dynamic_reconfigure::Server<manipulator_moveit_control::JointCtlParamsConfig>::CallbackType cb;
	public:
		JointControl();
		~JointControl();
